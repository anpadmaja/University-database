...
public class UniversityService implements Serializable{
	...
	public String getChargesDue(int personId) throws SQLException{
		...
		public String enrollStudent(int studentId, String courseId)throws Exception
		{
			...
			try
			{
				...
				message = gson.toJson(responseTO);
			}
			
			catch(Exception e){
	  		    responseTO.setError("ERROR");
	  		    message = gson.toJson(responseTO);
				e.printStackTrace();
			}
			finally {
	...
	public String createStudent(String details) throws Exception{
		...
		try
		{
			...
			String degreePursuing=studentto.getDegreePursuing();
			int estimatedGraduationYear=studentto.getEstimateGradutationYear();
			float gpa=studentto.getGpa();
			String flag="S";
			ResultSet rs=null;
			int personId = 0;
			String DBUsername = "";
			String fullName = fName+lName;
			StringTokenizer stoken = new StringTokenizer(fullName,"-");
			while(stoken.hasMoreElements()){
				DBUsername += stoken.nextToken();
			}
			
			//Setting the emailId and one time password for the student
			String emailId=DBUsername+"@cmpe273.edu";
			String password="Test1234";
			
			//Inserting student data into the person table
			String insertQuery= "insert into person(firstName,lastName,addressLine,state,city,zip,emailId,password,flag) values('"+fName+"','"+lName+"','"+address+"','"+state+"','"+city+"','"+zip+"','"+emailId+"','"+password+"','"+flag+"')";
			st = con.prepareStatement(insertQuery);
			if(st.executeUpdate() == 1)
			{
				//Inserting student data into the student table
				String selectQuery= "select personId from person where firstName = '"+fName+"' and lastName = '"+lName+"'";
				st = con.prepareStatement(selectQuery);
				rs=st.executeQuery();
				while(rs.next()){
				personId=rs.getInt(1);
				}
				String updateQuery="insert into student(studentId,departmentId,admitSemester,currentSemester,degreePursuing,estimatedYearOfGraduation,GPA) values('"+personId+"','"+deptId+"','"+admitSem+"','"+currentSem+"','"+degreePursuing+"','"+estimatedGraduationYear+"','"+gpa+"')";
	...
	public String removeStudentInstructor(int personId, String flag) throws SQLException{
		...
		try
		{
			...
			if(flag.equals("S")){
				query = "delete from studentenroll where studentId= "+personId+";";
				query1 = "delete from student where studentId="+personId+";";
				query2 = "delete from person where personId="+personId+";";
				System.out.println(">>>>>>>>>>>>"+query+"<<<<<<<<<<");
			}
			else{
				query = "delete from instructorscourse where personId= "+personId+";";
				query1 = "delete from instructor where personId= "+personId+";";
				query2 = "delete from person where personId="+personId+";";
			}
			st=con.prepareStatement(query);
			if(st.executeUpdate() == 1)
	...
	public String removeCourse(int courseId) throws SQLException{
		...
		try
		{
			...
			String query = "delete from studentenroll where courseId="+courseId+";";
			String query1 = "delete from instructorscourse where courseId="+courseId+";";
			String query2 = "delete from course where courseId="+courseId+";";
			st=con.prepareStatement(query);
			if(st.executeUpdate() == 1)
			{
				st=con.prepareStatement(query1);
	...
	public String createInstructor(String details) throws Exception{
		...
		try
		{
			...
			int deptId=instructorto.getDepartmentId();
			String officeRoom=instructorto.getOfficeRoom();
			String meetingHours=instructorto.getMeetingHours();
			...
			if(st.executeUpdate() == 1)
			{
				...
				String updateQuery="insert into instructor(personId,departmentId,meetingHours,officeRoom,dateOfJoining,numberOfClasses,position) values('"+personId+"','"+deptId+"','"+meetingHours+"','"+officeRoom+"','"+dateOfJoining+"','"+noOfClasses+"','"+position+"')";
				st=con.prepareStatement(updateQuery);
				if(st.executeUpdate()==1)
				{
				con.commit();
